...
export const Mobile_Support_Software = () => {
    const CATEGORY ='Mobiles_SoftWare_Context'

    const { setMobiles_SoftWare_Context } = useContext(videosContext)
    const [servicesArray, setservicesArray] = useState([])



    const stateObj =[
        { category:"Mobiles_SoftWare_Context", servicename: 'New Device Setup', selected: false, amount: '' },    
        { category:"Mobiles_SoftWare_Context", servicename: 'Device Support', selected: false, amount: '' }       
      ]
    ...
    const setDataToContext = () => {
        ...
        setMobiles_SoftWare_Context([...servicesArray])
    }

    return (
        <div>

            <h1 className='my-6 mt-14 text-xl font-semibold'>Mobile Support:</h1>
            <h1 className='my-6 text-lg font-semibold'>Software Support</h1>

            {/* ServiceItems */}
            {Mobiles_SoftWare.map((subcategory, index) => {
                return (
